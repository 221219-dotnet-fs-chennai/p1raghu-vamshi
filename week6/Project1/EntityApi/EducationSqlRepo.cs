...
namespace EntityApi
{
    public class EducationSqlRepo:IEducationRepo<Entities.Education>
    {
        ...
        {
          var search = context.Education.Where(t => t.UserId == UserId).FirstOrDefault();
            if (search != null)
            {
                context.Education.Remove(search);// this will generate DELETE query of Sql to be passed to Database
                context.SaveChanges();
            }
            return search;
