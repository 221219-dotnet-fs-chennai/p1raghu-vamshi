...
namespace ProjestLibrary
{
    public class SqlRepo : IFile
    {
        ...
        {
            string query = @"insert into Raghu.UserDetails (userId, Age, Salutation ,FirstName, MiddleName, LastName, Gender, Email) values (@userId, @Age, @Salutation, @FirstName, @MiddleName, @LastName, @Gender, @Email)";
            using SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            SqlCommand sqlCommand = new SqlCommand(query, con);
        ...
        public List<UserDetails> GetAllDetails()
        {
            ...
            try
            {
                ...
                //string query = "select Id, Name, OpenTime, CloseTime, Zipcode from Training.Restaurants";
                // for trainer
                string query = "select userId, Age, Salutation, FirstName, MiddleName,LastName,Gender,Email from Raghu.UserDetails";
                using SqlCommand command = new SqlCommand(query, con);
                // execute it
                using SqlDataReader reader = command.ExecuteReader();
                // process the output
